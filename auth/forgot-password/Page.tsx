    ...
    return (
        <div className="h-[100dvh] flex flex-col items-center justify-center px-[clamp(1rem,4vw,2rem)] relative overflow-hidden">
            ...
            >
                ...
                <h2 className="text-[clamp(1.25rem,2vw,1.6rem)] font-black text-center text-white tracking-tight mb-[clamp(1.5rem,4vh,2.5rem)] uppercase whitespace-nowrap">{uiCopy.auth.forgotPasswordTitle}</h2>
                ...
                {isSuccess ? (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center gap-4 text-center">
                        ...
                        <p className="text-slate-300 font-medium">{uiCopy.auth.forgotPasswordSuccess}</p>
                        <EliteButton variant="secondary" onClick={() => navigate('/login')} fullWidth title={uiCopy.auth.loginCta} className="mt-4" />
                    </motion.div>
                ) : (
                ...
                <p className="text-center text-slate-400 mt-10 text-sm font-medium">
                    {uiCopy.auth.forgotPasswordPrompt}{' '}
