        ...
        <main className="flex-grow flex items-center justify-center p-4 z-10">
          <div className="w-full max-w-md animate-fade-in-slide-up" style={{ animationDelay: '100ms' }}>
              ...
              <div className="bg-black/30 backdrop-blur-xl shadow-xl rounded-2xl p-8 border border-white/20">
                  <h2 className="text-2xl font-bold text-center text-white mb-6">{uiCopy.auth.registerTitle}</h2>
                  <form onSubmit={handleSubmit(handleRegister)} className="space-y-6">
                      ...
                      <div>
                          ...
                          <input
                              ...
                              className={`${inputClasses} ${errors.name ? 'border-red-500 focus:ring-red-500' : 'focus:ring-pink-500'}`}
                          />
                      ...
                      >
                          {isSubmitting ? uiCopy.auth.creatingAccount : uiCopy.auth.registerCta}
                      </button>
                  </form>
                  <p className="text-center text-slate-300 mt-6">
                      {uiCopy.auth.registerPrompt}{' '}
                      <button onClick={() => navigate('/login')} className="font-semibold text-pink-400 hover:text-pink-300 transition-colors duration-300">
