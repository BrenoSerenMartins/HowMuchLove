...
const BottomNavBar: React.FC<BottomNavBarProps> = ({ onMenuOpen, onLogoutRequest }) => {
  ...
  const loggedOutItems = [
    { path: '/login', label: 'Entrar', icon: <ArrowRightOnRectangleIcon className="w-5 h-5" /> },
    { action: () => handleScrollTo('pricing'), label: 'Planos', icon: <TagIcon className="w-5 h-5" /> },
    { action: () => handleScrollTo('demo'), label: 'Experimentar', icon: <SparklesIcon className="w-5 h-5" /> },
  ];

  const loggedInItems = [
    { path: '/dashboard', label: 'Nossa História', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg> },
    { path: '/settings', label: 'Conta', icon: <GearIcon className="w-5 h-5" /> },
    { action: onLogoutRequest, label: 'Sair', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg> },
  ];
