...
const TransparentCheckoutForm: React.FC<TransparentCheckoutFormProps> = ({ planId, planName, amount, onPaymentSuccess, onPaymentError, publicKey }) => {
  ...
  useEffect(() => {
    if (mercadoPago) {
      mercadoPago.getIdentificationTypes()
        ...
        .catch((err: any) => {
          logError('customer/settings/TransparentCheckoutForm.fetchIdentificationTypes', err, { publicKeyLoaded: !!publicKey });
          setError(getErrorMessage(err, uiCopy.payment.checkoutUnavailable));
        })
        .finally(() => {
          setIsSdkReady(true);
        });
  ...
  useEffect(() => {
    ...
    if (isSdkReady && mercadoPago && !cardFormRef.current) {
      const form = mercadoPago.cardForm({
        ...
        form: {
          id: "form-checkout",
          cardholderName: { id: "form-checkout__cardholderName", placeholder: uiCopy.payment.cardholderPlaceholder },
          identificationType: { id: "form-checkout__identificationType" },
          identificationNumber: { id: "form-checkout__identificationNumber" },
          cardNumber: { id: "form-checkout__cardNumber", placeholder: " " }, // Placeholder vazio para evitar sobreposição
          cardExpirationDate: { id: "form-checkout__cardExpirationDate", placeholder: uiCopy.payment.cardExpirationPlaceholder },
          securityCode: { id: "form-checkout__securityCode", placeholder: uiCopy.payment.securityCodePlaceholder },
          installments: { id: "form-checkout__installments" },
          issuer: { id: "form-checkout__issuer" },
        },
        callbacks: {
          onFormMounted: (err: any) => { if (err) console.warn('Form Mounted error:', err); },
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      logError('customer/settings/TransparentCheckoutForm.createCardToken', err, { planId, planName });
      ...
      const message = Array.isArray(err)
        ? uiCopy.payment.formFieldError
        : getErrorMessage(err, uiCopy.payment.genericError);
      setError(message);
  ...
  return (
    <div className="w-full mx-auto">
      ...
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold text-white">{uiCopy.payment.checkoutTitle}</h2>
        <p className="text-base text-slate-300 mt-1">
          {uiCopy.payment.planLabel}{' '}
          <span className="font-semibold text-pink-400">{planName}</span> - {uiCopy.payment.totalLabel}:{' '}
          <span className="font-semibold text-pink-400">R$ {amount.toFixed(2)}</span>
        </p>
      </div>
      ...
      {!isSdkReady ? (
        ...
        <>
          ...
          <form id="form-checkout" className="space-y-6" onSubmit={handleSubmit}>
            <div>
              <input type="text" id="form-checkout__cardholderName" className={`${inputClasses} ${getFieldClass('cardholderName')}`} placeholder={uiCopy.payment.cardholderPlaceholder} />
            </div>
            <div className="relative">
              ...
              {cardThumbnail && <img src={cardThumbnail} alt={uiCopy.payment.cardBrandAlt} className="absolute right-3 top-1/2 -translate-y-1/2 h-5 w-auto" />}
            </div>
            ...
            <div>
              <input type="text" className={`${inputClasses} ${getFieldClass('identificationNumber')}`} placeholder={uiCopy.payment.documentPlaceholder} value={maskIdentificationNumber(identificationNumber)} onChange={handleIdentificationNumberChange} />
            </div>
