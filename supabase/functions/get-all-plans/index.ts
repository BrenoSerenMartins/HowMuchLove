...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: plans, error } = await supabaseClient.from('plans').select('id, name, price, image_limit, allow_youtube, allow_password_protection, allow_custom_button, features, billing_cycle, is_featured').eq('is_active', true).order('price', {
      ascending: true
    });
    if (error) {
